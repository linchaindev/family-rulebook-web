...
// ── Yabaui target calculator ──────────────────────────────────────────────────
function calcYabawiTarget(
  items: string[],
  sliceAngle: number,
): { winIdx: number; targetAngle: number } {
  ...
  // Sector i spans angle ∈ [-(i+1)*sliceAngle, -i*sliceAngle]
  // Boundary between (i-1) and i is at angle = -i*sliceAngle
  type Cand = { winIdx: number; targetAngle: number };
  const candidates: Cand[] = [];

  for (const prizeIdx of prizeIndices) {
    const danger = (2 + Math.random() * 7) * (Math.PI / 180); // 2–9°
    // Sector just BEFORE prize: (prizeIdx-1), stop just inside near prize boundary
    const before = (prizeIdx - 1 + n) % n;
    if (!prizeSet.has(before)) {
      // Prize left boundary = -prizeIdx*sliceAngle → stop at +danger (inside before sector)
      candidates.push({ winIdx: before, targetAngle: -prizeIdx * sliceAngle + danger });
    }
    // Sector just AFTER prize: (prizeIdx+1), stop just inside near prize boundary
    const after = (prizeIdx + 1) % n;
    if (!prizeSet.has(after)) {
      // Prize right boundary = -(prizeIdx+1)*sliceAngle → stop at -danger (inside after sector)
      candidates.push({ winIdx: after, targetAngle: -(prizeIdx + 1) * sliceAngle - danger });
    }
  }
...
// ── Render label with bold-red "상" when yabaui active ─────────────────────
function renderLabel(item: string, yabawiActive: boolean): React.ReactNode {
  if (!yabawiActive || !item.includes('상')) return item;
  const parts = item.split('상');
  return parts.flatMap((part, i) =>
    i < parts.length - 1
      ? [part, <strong key={i} style={{ color: '#dc2626' }}>상</strong>]
      : [part],
  );
}
...
export default function RouletteGame() {
  ...
  const drawWheel = useCallback(
    (angle: number, highlightIdx: number | null, yabawiActive: boolean) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const isPrize   = yabawiActive && items[i].includes('상');

        // Radial gradient: bright inner → natural color outer
        ...
        ctx.lineWidth   = isWinner ? 3.5 : (isPrize ? 2.5 : 1.5);
        ctx.stroke();

        // Prize sector: red glow border
        if (isPrize) {
          ctx.beginPath();
          ctx.moveTo(cx, cy);
          ctx.arc(cx, cy, radius, startA, endA);
        ...
        if (yabawiActive && label.includes('상')) {
          // Draw parts with bold-pink "상"
          const parts = label.split('상');
          ctx.textAlign = 'left';
          ctx.font = `${fontSize}px sans-serif`;
          const w0  = ctx.measureText(parts[0]).width;
          ctx.font  = `bold ${fontSize * 1.1}px sans-serif`;
          const ws  = ctx.measureText('상').width;
          ctx.font  = `${fontSize}px sans-serif`;
          const w1  = parts[1] ? ctx.measureText(parts[1]).width : 0;
          let x     = -(w0 + ws + w1) / 2;

          ctx.fillStyle = 'white';
          ctx.fillText(parts[0], x, -textR);
          x += w0;

          ctx.font      = `bold ${fontSize * 1.1}px sans-serif`;
          ctx.fillStyle = '#fca5a5'; // light red
          ctx.fillText('상', x, -textR);
          x += ws;

          if (parts[1]) {
            ctx.font      = `${fontSize}px sans-serif`;
            ctx.fillStyle = 'white';
            ctx.fillText(parts[1], x, -textR);
          }
        } else {
      ...
      // ── Yabaui pulse ring ──────────────────────────────────────────────────
      if (yabawiActive) {
        const pulse = 0.28 + 0.22 * Math.sin(Date.now() / 260);
        ctx.beginPath();
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      drawWheel(wheelAngleRef.current, winnerIndex, yabawiModeRef.current);
    };
    resize();
  ...
  useEffect(() => {
    drawWheel(wheelAngleRef.current, winnerIndex, yabawiModeRef.current);
  }, [items, winnerIndex, drawWheel, yabawiMode]);
  ...
  const handleSpin = useCallback(() => {
    ...
    // ── Yabaui activation (second click while spinning) ──────────────────────
    if (phase === 'spinning') {
      if (yabawiModeRef.current) return; // already active

      const prizeItems = items.filter(it => it.includes('상'));
      if (prizeItems.length === 0) return; // nothing to avoid
      ...
      // Ensure at least 2 full revolutions from current position
    ...
    const animate = (now: number) => {
      ...
      // Boundary detection → pin bounce + tick sound
      ...
      if (bCount !== lastBoundaryCountRef.current) {
        ...
        pinBounceRef.current = { startTime: now, direction: Math.random() < 0.5 ? 1 : -1, duration: bounceDur };
        playTick(angVel > 5);
      }
      ...
      draw(angle, null, yabawiModeRef.current);
      ...
      if (progress < 1) {
        ...
        draw(finalAngle, params.winIdx, yabawiModeRef.current);
  ...
  const handleReset = useCallback(() => {
    ...
    drawWheel(wheelAngleRef.current, null, false);
  }, [drawWheel]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Item list (setup + result) */}
      {phase !== 'spinning' && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-sm">룰렛 항목</h3>
            {phase === 'setup' && (
              <Button size="sm" variant="outline" onClick={addItem}>+ 항목 추가</Button>
            )}
          </div>
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <span
                  ...
                  style={{ backgroundColor: ROULETTE_COLORS[idx % ROULETTE_COLORS.length] }}
                />
                {phase === 'result' ? (
                  <div
                    className="flex-1 h-8 text-sm flex items-center px-3 rounded border"
                    style={{
                      borderColor : winnerIndex === idx ? winColor : 'transparent',
                      fontWeight  : winnerIndex === idx ? 700 : 400,
                      color       : winnerIndex === idx ? winColor : undefined,
                    }}
                  >
                    {renderLabel(item, yabawiMode)}
                  </div>
                ) : (
                  <Input
                    value={item}
      ...
      <div ref={containerRef} className="relative flex justify-center" style={{ paddingTop: '34px' }}>
        ...
        <canvas
          ...
          style={{
            borderRadius : '50%',
            display      : 'block',
            boxShadow    : yabawiMode
              ? '0 6px 32px rgba(220,38,38,0.5), 0 0 50px rgba(220,38,38,0.2)'
      ...
      {phase === 'result' && result && (
        <div
          key={result}
          className="text-center py-5 rounded-xl border-2 relative overflow-hidden"
          style={{
        ...
        >
          <div className="text-3xl mb-1" style={{ display: 'inline-block', animation: 'resultBounce 0.6s ease infinite alternate' }}>
            🎉
          </div>
          <div className="text-xl font-bold" style={{ color: winColor }}>
            {renderLabel(result, yabawiMode)}
          </div>
      ...
      {phase === 'spinning' && (
        /* disabled 제거 — 클릭 시 야바위 모드 활성화 (피드백 없음) */
        <Button
          className="w-full"
          size="lg"
          onClick={handleSpin}
          style={{
            background   : yabawiMode ? 'linear-gradient(135deg,#dc2626,#7f1d1d)' : 'linear-gradient(135deg,#667eea,#764ba2)',
            border       : 'none',
            color        : 'white',
            opacity      : 0.78,
            cursor       : 'default',
